...
export default function AdminDashboard({ lang }: { lang: Lang }) {
  ...
  const handleConfirmInstallation = async (token: string) => {
    try {
      const res = await fetch("/api/install/confirm", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ token })
      });
      if (res.ok) {
        window.location.href = "/admin?installed=true";
      } else {
        const data = await res.json();
        alert(data.error);
  ...
  const handleStartSetup = async () => {
    ...
    try {
      ...
      setInstallError("Server communication error");
      setInstallStatus("error");
    }
  };
  ...
  const handleChangePassword = async () => {
    if (newPassword !== confirmPassword) {
      setAuthError(lang === "sk" ? "Heslá sa nezhodujú" : "Passwords do not match");
      return;
    }
    if (newPassword.length < 6) {
      setAuthError(lang === "sk" ? "Heslo musí mať aspoň 6 znakov" : "Password must be at least 6 characters");
      return;
    }
    try {
      ...
      if (res.ok) {
        ...
        setIsAuthorized(true);
        setAuthError("");
        setPassword(newPassword); // Update password in state
      } else {
        const data = await res.json();
        setAuthError(data.error);
      }
  ...
  const handleBulkUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setStressStatus("uploading");
    setStressResults({ count: 0, details: [] });
    ...
    for (const file of files) {
      ...
      // Pridáme metadáta ako pri bežnej registrácii
      formData.append('author', 'Admin Bulk');
      formData.append('email', 'admin@sss.sk');
      formData.append('photoInfo', JSON.stringify([{ category: 'A', name: file.name, description: 'Bulk upload' }]));

      try {
        const response = await fetch('/api/register', { // Použijeme overený register endpoint
          method: 'POST',
          body: formData
        });
        ...
        if (response.ok) {
          successCount++;
          details.push({ file: file.name, status: 'ok' });
        } else {
          details.push({ file: file.name, status: 'error', message: data.error });
        }
      } catch (err) {
        details.push({ file: file.name, status: 'fatal' });
      }
      
      // Priebežná aktualizácia stavu
      setStressResults({ count: successCount, details });
    }
  ...
  if (!isAuthorized) {
    return (
      <div className="max-w-md mx-auto mt-20 p-8 border border-gray-200 bg-white shadow-xl rounded-2xl space-y-6">
        <div className="space-y-2 text-center">
          <Shield size={32} className="mx-auto text-blue-600 mb-4" />
        ...
        <div className="space-y-4">
          <div className="space-y-1">
            ...
            <input 
              ...
              className="w-full p-3 border border-gray-200 bg-gray-50 text-sm outline-none focus:border-blue-500 rounded-xl"
              placeholder="admin"
            />
          </div>
          
          <div className="space-y-1">
            <label className="text-[10px] font-bold uppercase text-gray-400 ml-1">{lang === "sk" ? "Heslo" : "Password"}</label>
            <input 
              type="password" 
              value={password}
          ...
          <div className="pt-2">
            <button 
              onClick={handleLogin}
              className="w-full py-4 bg-gray-900 text-white font-bold uppercase tracking-[3px] text-[11px] transition-opacity hover:opacity-90 rounded-xl"
            >
